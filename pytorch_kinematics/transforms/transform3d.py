...
class Transform3d:
    ...
    def __init__(
        ...
        if matrix is None:    
            if pos is not None or rot is not None:
                # Build from pos and rot
                if rot is None:
                    rot = torch.eye(3, dtype=dtype, device=device).view(default_batch_size, 3, 3)
                if pos is None:
                    pos = torch.zeros(3, dtype=dtype, device=device).view(default_batch_size, 3)
                ...
                rot = rot.view(-1, 3, 3)
                pos = pos.view(-1, 3)
                batch_size = max(rot.shape[0], pos.shape[0])
                if rot.shape[0] != batch_size:
                    rot = rot.repeat(batch_size, 1, 1)
                ...
                self._matrix = torch.cat(                    
                    [
                        ...
                        torch.tensor([0, 0, 0, 1], dtype=dtype, device=device).view(1, 1, 4).repeat(batch_size, 1, 1)
                    ],
                    dim=1)
    ...
    def grad_zero(self):
        """
        Return a new Transform3d object with the same transformation matrix
        but with the gradients set to zero.
        """
        if self._matrix.grad is not None:
            self._matrix.grad.data.zero_()
