...
def _convert_collision(collision):
    if collision is None or collision.geometry is None:
        return frame.Visual()
    else:
        c_tf = _convert_transform(collision.origin)
        if isinstance(collision.geometry, Mesh):
            g_type = "mesh"
            g_param = collision.geometry.filename
        elif isinstance(collision.geometry, Cylinder):
            g_type = "cylinder"
            g_param = (collision.geometry.radius, collision.geometry.length)
        elif isinstance(collision.geometry, Box):
            g_type = "box"
            g_param = collision.geometry.size
        elif isinstance(collision.geometry, Sphere):
            g_type = "sphere"
            g_param = collision.geometry.radius
        else:
            g_type = None
            g_param = None
        return frame.Visual(c_tf, g_type, g_param)
...
def _build_chain_recurse(root_frame, lmap, joints):
    ...
    for j in joints:
        if j.parent == root_frame.link.name:
            ...
            if j.limit is None and j.type != "fixed":
                raise ValueError("Joint limits are not specified for joint", j.name)
            child_frame.joint = frame.Joint(j.name, offset=_convert_transform(j.origin),
                                            joint_type=JOINT_TYPE_MAP[j.type], axis=j.axis, jrange=[j.limit.lower, j.limit.upper] if j.limit is not None else None)
            link = lmap[j.child]
            child_frame.link = frame.Link(link.name, offset=_convert_transform(link.origin),
                                          visuals=[_convert_visual(v) for v in link.visuals], collisions=[_convert_collision(c) for c in link.collisions])
...
def build_chain_from_urdf(data):
    ...
    root_frame.link = frame.Link(root_link.name, _convert_transform(root_link.origin),
                                 [_convert_visual(v) for v in root_link.visuals], [_convert_collision(c) for c in root_link.collisions])
